<template>
  <AdminLayout :title="'Edit ' + role.display_name">
    <div class="space-y-6">
      ...
      <div>
        <nav class="flex items-center gap-2 text-[12px] text-gray-500 mb-3">
          ...
          <span class="text-gray-300">Edit {{ role.display_name }}</span>
        </nav>
        <h1 class="text-[22px] font-bold text-white tracking-tight">Edit {{ role.display_name }}</h1>
        <p class="mt-0.5 text-[13px] text-gray-400">Modify permissions and settings for this role</p>
      </div>
        ...
        <form @submit.prevent="submit" class="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              type="text"
              readonly
              class="w-full h-10 px-3.5 rounded-xl bg-white/[0.02] border border-white/[0.05] text-[13px] text-gray-500 cursor-not-allowed"
            />
            <p class="mt-1.5 text-[11px] text-gray-500">Role name cannot be changed</p>
          </div>
...
const submit = () => form.patch(route('admin.roles.update', props.role.id))
</script>
